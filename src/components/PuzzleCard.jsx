...
export default function PuzzleCard({ puzzle, onSolve }) {
  ...
  return (
    <div className="p-6 bg-black/70 border border-green-600 rounded-2xl text-green-200 max-w-xl mx-auto font-mono shadow-xl">
      <h2 className="text-green-400 font-bold text-lg mb-2">{puzzle.title}</h2>
      <pre className="whitespace-pre-wrap text-sm mb-3 text-green-100">{puzzle.prompt}</pre>
      ...
      {feedback && <p className="mt-3 text-sm text-yellow-300 whitespace-pre-wrap">{feedback}</p>}
      {zetaResponse && <p className="mt-3 text-sm text-cyan-300 whitespace-pre-wrap">{zetaResponse}</p>}
      ...
      <div className="mt-4 flex justify-between items-center">
        ...
        <span className="text-xs text-green-500 italic">Hints used: {hintsUsed}</span>
      </div>
    </div>
  );
}